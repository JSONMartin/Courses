...
const CompanyType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    users: {
      type: new GraphQLList(UserType), // Bi-directional join type, "List of Users"
      resolve(parentValue, args) { // parentValue is current company
...
const UserType = new GraphQLObjectType({
    name: 'User', //Required, capitalized by convention
    fields: () => ({
      id: { type: GraphQLString },
...
const RootQuery = new GraphQLObjectType({
  ...
  fields: {
    user: {
      ...
      resolve(parentValue, args) { // parentValue is almost never used
    ...
    tag: {
      ...
      args: { id: { type: GraphQLString } },
      resolve(parentValue, args) {
        return axios.get(`http://localhost:3000/tags/${args.id}`).then(resp => resp.data)
      }
    }
  }
})
...
const mutation = new GraphQLObjectType({
  ...
  fields: {
    addUser: {
      type: UserType, // Doesn't always return the same type as working with
      args: {
